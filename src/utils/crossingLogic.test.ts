...
describe('calculateCrossingStatus', () => {
  ...
  it('should return CLOSED when a train is arriving imminently', () => {
    ...
    expect(status.nextTrain?.minutesUntil).toBe(2);
  });

  it('should return WARNING when a train is departing soon but not imminent', () => {
    const now = createTime(10, 0);
    const data: FerrotramviariaResponse = {
      arrivi: [],
      partenze: [
  ...
  it('should return WARNING when a train is arriving in 6 minutes', () => {
    const now = createTime(10, 0);
    const data: FerrotramviariaResponse = {
      arrivi: [
        {
    ...
    const status = calculateCrossingStatus(data, now);
    expect(status.state).toBe('WARNING');
    expect(status.nextTrain?.minutesUntil).toBe(6);
  });
  ...
  it('should keep CLOSED right after a train has just passed', () => {
    const now = createTime(10, 0);
    const data: FerrotramviariaResponse = {
      arrivi: [
        {
    ...
    const status = calculateCrossingStatus(data, now);
    expect(status.state).toBe('CLOSED');
    expect(status.nextTrain?.minutesUntil).toBe(-1);
  });
